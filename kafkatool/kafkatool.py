...
def print_offsets(client, args):
    ...
    NOTE: Time-based offset lookups are not precise, but are based on segment
          boundaries. Will return the latest offset on very recently created
          topics.
...
def create_topic(client, args):
    """Create new topic with configuration from arguments"""
...
def delete_topic(client, args):
    """Delete an existing topic, use with caution"""
...
def _get_arg_parser():
    output = argparse.ArgumentParser(description='CLI tool for Kafka.')
    ...
    output.add_argument('-s', '--ssl-auth',
                        required=False,
                        default='false',
                        dest='ssl_enabled',
                        help='Use SSL authentication. Set to true or false. '
                             'Provide CA, client cert and key file via environment variables. '
                             '[default: false]')
...
def main():
    ...
    if args.command:
        if args.ssl_enabled is "true":
            config = SslConfig(cafile=os.environ['KAFKA_SSL_CA_CERT'],
                               certfile=os.environ['KAFKA_SSL_CLIENT_CERT'],
                               keyfile=os.environ['KAFKA_SSL_CLIENT_KEY'])
            client = pykafka.KafkaClient(hosts=args.host, broker_version=args.broker_version, ssl_config=config)
        else:
